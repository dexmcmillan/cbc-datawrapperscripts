...
print(raw['coordinates'].dtype)
raw['x'] = raw["coordinates"].apply(lambda x: re.search(x, "\[[0-9]+\.[0-9]+").replace("\[", ""))
raw['x'] = raw["coordinates"].apply(lambda x: re.search(x, "[0-9]+\.[0-9]+\]").replace("\]", ""))
print(raw['x'])
# raw['coordinates'] = raw['coordinates'].astype(list).apply(lambda x: Point(int(x[0]), int(x[1])))

raw = geopandas.GeoDataFrame(raw, geometry='coordinates')
...
dw = (chart
      .upload(data)
      .title("Russia launches offensive in Eastern Ukraine (TEST)")
      .timestamp()
      )
