...
(datawrappergraphics.Chart(MAP_ID)
 .data(data)
 .head(f"There are at least <b>{num_fires:,} wildfires</b> burning in Canada right now, covering {total_area:,} hectares")
 .deck(f"The longest has been burning for <b>{longest_duration}</b>, and <b>{percent_oc}%</b> are classifed as out of control.<br><br>Size of the circles indicates relative size of the fire. Zoom in to see fires that are closely grouped.")
 .footer(source=f"Natural Resources Canada", byline="Dexter McMillan", timestamp=True)
 .publish()
 )